...
do
  CMD=$( $GLOBALPATH/bin/cleos.sh system newaccount $NAME $account $PUB_KEY --stake-net "100.0000 EOS" --stake-cpu "100.0000 EOS" --buy-ram "1.0000 EOS" --transfer 2>$tpm_stderr)
  ERR=$(cat $tpm_stderr)
