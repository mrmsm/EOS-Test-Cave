...
#
###############################################################################
TEST_NAME="Wallet create_key K1 in $NAME wallet"
...
#--------------------------------------------------
CMD1=$($GLOBALPATH/bin/cleos.sh wallet create_key -n $NAME K1 2>$tpm_stderr)
...
if [[ "$CMD" == *"$PUB_KEY"* ]]; then
    echo "$PUB_KEY" >> $GLOBALPATH/log/wallet_create_key_name_k1.dat
    echo "1:$TEST_NAME"
else
